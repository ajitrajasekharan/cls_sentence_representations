...
class CL_SE:
    ...
    def output_vectors_test(self,sent,is_patched):
        #sent = sent.lower().replace("entity","[MASK]")
        #sent = sent.replace("entity","[MASK]")
        text = '[CLS]' + sent + '[SEP]'
        ...
        for vec_i in arr:
            ...
            for term in  self.terms_dict:
                ...
                #print(tokenized_text[i],tokenized_text[j],val)
                tok2 = term
...
def main(model_path,vector_file,vocab_file):
    is_mlm = True
    is_patched = True
    ...
    while (True):
        ...
        sent = input()
        se.output_vectors_test(sent,True)
...
if __name__ == "__main__":
    if (len(sys.argv) != 4):
        print("Usage prog <model path> <vector file> <vocab file>")
    else:
